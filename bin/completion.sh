...
remove_stale_completion_entries() {
    ...
    if [[ ! -f "$config_file" ]] || ! grep -Eq "(^# Mole-AI shell completion$|(mole|mo)[[:space:]]+completion)" "$config_file" 2> /dev/null; then
        return 1
    fi
    ...
    temp_file="$(mktemp)"
    grep -Ev "(^# Mole-AI shell completion$|(mole|mo)[[:space:]]+completion)" "$config_file" > "$temp_file" || true
...
if [[ $# -eq 0 ]]; then
    ...
    if [[ "$current_shell" == "fish" ]]; then
        ...
        if [[ -z "$completion_name" ]]; then
            ...
            log_error "mole-ai not found in PATH, install Mole-AI before enabling completion"
            exit 1
        fi
        ...
        printf '# Mole-AI completions for mo (alias) -- auto-generated, do not edit\n' > "$mo_file"
        printf 'source %s\n' "$mole_file" >> "$mo_file"
    ...
    # Remove previous Mole-AI completion lines to avoid duplicates
    if [[ -f "$config_file" ]]; then
        original_mode=""
        original_mode="$(stat -f '%Mp%Lp' "$config_file" 2> /dev/null || true)"
        temp_file="$(mktemp)"
